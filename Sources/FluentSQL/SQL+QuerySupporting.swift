...
extension QuerySupporting where Query: FluentSQLQuery, Output == Query.RowDecoder.Row {
    /// See `QuerySupporting`.
    public static func queryDecode<D>(_ output: Output, entity: String, as decodable: D.Type, on conn: Connection) -> Future<D> where D : Decodable {
        do {
            let row = try Query.RowDecoder.init().decode(D.self, from: output, table: .table(.identifier(entity)))
